...
export default function Navbar({ sidebarOpen, setSidebarOpen }) {
  ...
  const navigation = [
    ...
    // Show Resume link for unauthenticated users OR for job seekers (non-employer/admin logged in users)
    ...((!user || (!isEmployer() && !isAdmin())) ? [{ name: 'Resume', href: '/resume', icon: FaFileAlt }] : []),
            ...
            <div className="hidden md:flex items-center space-x-1">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${router.pathname === item.href
                ...
                <div className="flex flex-col space-y-1">
                  {navigation.map((item) => (
                    <Link
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${router.pathname === item.href
                        ? 'text-white bg-white/10'
                        : 'text-gray-300 hover:text-white hover:bg-white/5'
                        }`}
                    >
